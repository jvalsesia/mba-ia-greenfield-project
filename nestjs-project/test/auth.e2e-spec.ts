...
import {
  ...
  type ApiErrorBody,
  type RegisteredUserBody,
  type TokenPairBody,
} from '../src/test/api-response';
...
describe('Auth (e2e)', () => {
  ...
  describe('POST /auth/register', () => {
    ...
    it('returns 409 with EMAIL_ALREADY_EXISTS on duplicate email', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('EMAIL_ALREADY_EXISTS');
    });
  ...
  describe('GET /auth/confirm-email', () => {
    ...
    it('returns 401 with INVALID_TOKEN on an already-used token', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('INVALID_TOKEN');
    });
    ...
    it('returns 401 with TOKEN_EXPIRED on an expired token', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('TOKEN_EXPIRED');
    });
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect(bodyOf<AccessTokenPayload>(res).sub).toBeDefined();
      expect(bodyOf<AccessTokenPayload>(res).email).toBe('me@example.com');
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 401 with INVALID_CREDENTIALS on wrong password', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('INVALID_CREDENTIALS');
    });
    ...
    it('returns 403 with EMAIL_NOT_CONFIRMED when user is not confirmed', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('EMAIL_NOT_CONFIRMED');
    });
  ...
  describe('POST /auth/refresh', () => {
    ...
    it('returns 401 with TOKEN_REUSE_DETECTED when reuse is beyond grace period', async () => {
      ...
      expect(bodyOf<ApiErrorBody>(res).error).toBe('TOKEN_REUSE_DETECTED');
